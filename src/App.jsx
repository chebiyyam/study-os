...
const COURSES = {
  ...
  STAT211: {
    ...
    color: "#D4A843", sem: "Fall 2026",
    resource: { label: "Crawford YouTube Series", url: "https://www.youtube.com/@CrawfordStats" },
    weeks: [
...
const Icon = ({ n, size = 16 }) => {
  const icons = {
    book: <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 19.5A2.5 2.5 0 016.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z"/></svg>,
    timer: <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>,
    ...
    tasks: <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>,
    external: <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>,
    chevron: <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>,
    ...
    trash: <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 011-1h4a1 1 0 011 1v2"/></svg>,
  };
  return icons[n] || null;
};
...
function CourseHub() {
  ...
  const setWS = (key, val) => { const n = {...weekStatus, [key]: val}; setWeekStatus(n); save("weekStatus", n); };
  const setHS = (key, val) => { const n = {...hwStatus, [key]: val}; setHwStatus(n); save("hwStatus", n); };
  const setNote = (key, val) => { const n = {...notes, [key]: val}; setNotes(n); save("courseNotes", n); };
  ...
  const fallCourses = Object.values(COURSES).filter(c => c.sem === "Fall 2026");
  const springCourses = Object.values(COURSES).filter(c => c.sem === "Spring 2027");
    ...
    <div style={{ display: "flex", gap: 24, height: "100%" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", paddingRight: 4 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
          <div>
            <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 11, color: color, marginBottom: 4 }}>{course.code} · {course.sem}</div>
          ...
          {course.resource && (
            <a href={course.resource.url} target="_blank" rel="noreferrer" style={{
              ...
              textDecoration: "none", border: `1px solid ${color}30`, borderRadius: 8, padding: "6px 12px",
              background: `${color}10`,
            }}>
              {course.resource.label} <Icon n="external" size={12} />
            </a>
          )}
        </div>
        ...
        <div style={{ marginBottom: 6 }}>
          <div style={{ fontSize: 11, color: "#6B6B70", marginBottom: 10 }}>Weekly Progress</div>
            ...
            {course.weeks.map(w => {
              ...
              return (
                <button key={w.n} onClick={() => setExpandedWeek(isOpen ? null : w.n)} style={{
                  ...
                  background: bg, border: `1px solid ${st ? "transparent" : "#2A2A2E"}`,
          ...
          <div style={{ display: "flex", gap: 16, marginTop: 10, fontSize: 11, color: "#6B6B70" }}>
            {Object.entries(STATUS_COLOR).map(([k,v]) => <span key={k} style={{ display: "flex", alignItems: "center", gap: 5 }}><span style={{ width: 10, height: 10, borderRadius: 3, background: k ? v : "#1E1E22", border: k ? "none" : "1px solid #2A2A2E", display: "inline-block" }}/>{STATUS_LABEL[k]}</span>)}
          ...
          return (
            <div style={{ background: "#141416", border: "1px solid #252528", borderRadius: 12, padding: 20, marginTop: 16 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {STATUS_CYCLE.map(s => (
                    <button key={s} onClick={() => setWS(wKey, s)} style={{
                      padding: "4px 10px", borderRadius: 6, border: `1px solid ${st === s ? STATUS_COLOR[s] : "#2A2A2E"}`,
                      ...
                      {probs.map(p => {
                        ...
                        return (
                          <button key={p} onClick={() => setHS(hkey, !done)} style={{
                            padding: "4px 10px", borderRadius: 6, fontSize: 12, fontFamily: "JetBrains Mono, monospace",
                            border: `1px solid ${done ? color : "#2A2A2E"}`,
                            background: done ? `${color}25` : "transparent",
                            color: done ? color : "#A8A8B0", cursor: "pointer", transition: "all 0.12s",
              ...
              <div>
                ...
                <textarea
                  value={notes[wKey] || ""}
                  onChange={e => setNote(wKey, e.target.value)}
                  placeholder="Add notes for this week..."
                  style={{
                    width: "100%", minHeight: 80, background: "#0D0D0F",
                    border: "1px solid #252528", borderRadius: 8, padding: 12,
                    color: "#E8E8ED", fontSize: 13, resize: "vertical", fontFamily: "inherit",
...
function FocusTimer() {
  ...
  const totalSecs = useCustom
    ? (parseInt(custom) || 0) * 60
    : PRESETS[preset].mins * 60;
  ...
  return (
    <div style={{ maxWidth: 560 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        {PRESETS.map((p, i) => (
          <button key={i} onClick={() => { setPreset(i); setUseCustom(false); if (!running) setRemaining(null); }} style={{
            padding: "8px 16px", borderRadius: 8, border: `1px solid ${!useCustom && preset === i ? "#7C6FCD" : "#2A2A2E"}`,
        ...
        <div style={{ display: "flex", gap: 0, border: `1px solid ${useCustom ? "#7C6FCD" : "#2A2A2E"}`, borderRadius: 8, overflow: "hidden" }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontSize: 11, color: "#6B6B70", marginBottom: 8 }}>Tag session to course</div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {Object.values(COURSES).map(c => (
            <button key={c.id} onClick={() => setCourse(c.id)} style={{
              ...
              color: course === c.id ? c.color : "#6B6B70", cursor: "pointer",
        ...
        <div style={{ marginTop: 32 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {log.slice(0, 10).map(s => (
              <div key={s.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", background: "#141416", borderRadius: 8, border: "1px solid #1E1E22" }}>
...
function GpaCalc() {
  ...
  const cfg = configs[selected] || { categories: [{ name: "Exams", weight: 60, scores: [] }, { name: "Homework", weight: 20, scores: [] }, { name: "Quizzes", weight: 20, scores: [] }] };
  ...
  const updateCat = (i, field, val) => {
    const cats = cfg.categories.map((c, j) => j === i ? { ...c, [field]: val } : c);
    saveCfg({ ...cfg, categories: cats });
  };

  const addScore = (i, val) => {
    const cats = cfg.categories.map((c, j) => j === i ? { ...c, scores: [...c.scores, val] } : c);
    saveCfg({ ...cfg, categories: cats });
  };

  const removeScore = (i, si) => {
    const cats = cfg.categories.map((c, j) => j === i ? { ...c, scores: c.scores.filter((_,k) => k !== si) } : c);
    saveCfg({ ...cfg, categories: cats });
  };
  ...
  return (
    <div style={{ maxWidth: 600 }}>
      <h2 style={{ margin: "0 0 20px", fontSize: 20, fontWeight: 600, color: "#E8E8ED" }}>GPA Calculator</h2>

      {/* course picker */}
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 24 }}>
        {Object.values(COURSES).map(c => (
          <button key={c.id} onClick={() => setSelected(c.id)} style={{
            padding: "6px 12px", borderRadius: 8, border: `1px solid ${selected === c.id ? c.color : "#2A2A2E"}`,
      ...
      <div style={{ marginBottom: 16 }}>
        {cfg.categories.map((cat, i) => {
          ...
          const avg = cat.scores.length ? cat.scores.reduce((a,b) => a+b,0)/cat.scores.length : null;
          return (
            <div key={i} style={{ background: "#141416", border: "1px solid #1E1E22", borderRadius: 12, padding: 16, marginBottom: 10 }}>
              <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 12 }}>
                <input value={cat.name} onChange={e => updateCat(i,"name",e.target.value)} style={{ flex: 1, background: "transparent", border: "none", color: "#E8E8ED", fontSize: 14, fontWeight: 500, outline: "none" }} />
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                ...
                <input value={scoreInput} onChange={e => setScoreInput(e.target.value)}
                  ...
                  placeholder="add score…" style={{ width: 90, background: "transparent", border: "1px dashed #2A2A2E", borderRadius: 6, padding: "3px 8px", color: "#E8E8ED", fontSize: 12, outline: "none" }} />
              </div>
            </div>
          );
        })}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ fontSize: 12, color: totalW === 100 ? "#4CAF8A" : "#E07C54" }}>
            {totalW.toFixed(0)}% {totalW !== 100 ? "(weights must sum to 100)" : "✓"}
          </div>
        </div>
      </div>
      ...
      <div style={{ background: "#141416", border: "1px solid #252528", borderRadius: 12, padding: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <span style={{ fontSize: 13, color: "#A8A8B0" }}>Target final grade:</span>
        ...
        {currentGrade !== null && (
          <div style={{ marginTop: 12, fontSize: 13, color: "#E8E8ED" }}>
            {currentGrade >= target
              ? <span style={{ color: "#4CAF8A" }}>You're already at {currentGrade.toFixed(1)}% — above your target.</span>
              : <span>You need to average <span style={{ color: course.color, fontWeight: 600 }}>{((target - currentGrade) * (totalW / (100 - totalW || 1))).toFixed(1)}%+</span> on remaining work.</span>
            }
          </div>
        )}
      </div>
...
function WeeklyTasks() {
  ...
  const toggle = id => {
    ...
    setTasks(n); save("weeklyTasks", n);
  };

  const addTask = () => {
    if (!newTask.trim()) return;
    const t = { id: Date.now(), week: filterWeek || 0, dates: "Custom", text: newTask.trim(), done: false, course: newCourse };
    const n = [...tasks, t];
    setTasks(n); save("weeklyTasks", n);
    setNewTask("");
  };
  ...
  const nowWeek = (() => {
    ...
    const diff = Math.floor((today - base) / (7 * 24 * 60 * 60 * 1000)) + 1;
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        ...
        <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 11, color: "#6B6B70" }}>
          Current: Week {nowWeek} of 13
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginBottom: 20 }}>
        <button onClick={() => setFilterWeek(null)} style={{
          padding: "5px 12px", borderRadius: 6, border: `1px solid ${filterWeek === null ? "#7C6FCD" : "#2A2A2E"}`,
          background: filterWeek === null ? "#7C6FCD20" : "transparent", color: filterWeek === null ? "#7C6FCD" : "#6B6B70", fontSize: 12, cursor: "pointer",
        }}>All</button>
        {weeks.map(w => (
          <button key={w} onClick={() => setFilterWeek(filterWeek === w ? null : w)} style={{
            padding: "5px 12px", borderRadius: 6,
            border: `1px solid ${filterWeek === w ? "#7C6FCD" : w === nowWeek ? "#3A3A40" : "#1E1E22"}`,
            background: filterWeek === w ? "#7C6FCD20" : w === nowWeek ? "#1E1E22" : "transparent",
            color: filterWeek === w ? "#7C6FCD" : w === nowWeek ? "#E8E8ED" : "#6B6B70", fontSize: 12, cursor: "pointer",
            fontFamily: "JetBrains Mono, monospace",
          }}>{w === 0 ? "Custom" : `W${w}`}</button>
        ))}
      </div>
      ...
      {(() => {
        ...
        displayed.forEach(t => {
          const k = t.week;
          if (!grouped[k]) grouped[k] = { dates: t.dates, tasks: [] };
        ...
        return Object.entries(grouped).sort(([a],[b]) => parseInt(a)-parseInt(b)).map(([wk, g]) => {
          ...
          return (
            <div key={wk} style={{ marginBottom: 20 }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: 8, paddingBottom: 8, borderBottom: "1px solid #1E1E22" }}>
                <span style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 11, fontWeight: 700, color: parseInt(wk) === nowWeek ? "#7C6FCD" : "#6B6B70" }}>
                  {wk === "0" ? "Custom" : `Week ${wk}`}
                </span>
              ...
              {g.tasks.map(t => (
                <div key={t.id} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "7px 0", borderBottom: "1px solid #141416" }}>
                  <Checkbox checked={t.done} onChange={() => toggle(t.id)} color="#7C6FCD" />
                  <span style={{ flex: 1, fontSize: 13, color: t.done ? "#4A4A52" : "#C8C8D0", textDecoration: t.done ? "line-through" : "none", lineHeight: 1.4 }}>{t.text}</span>
                  <button onClick={() => del(t.id)} style={{ background: "none", border: "none", color: "#3A3A40", cursor: "pointer", padding: 2, flexShrink: 0 }}><Icon n="trash" size={13} /></button>
                </div>
              ))}
            </div>
          );
      ...
      <div style={{ marginTop: 24, display: "flex", gap: 8, background: "#141416", border: "1px solid #252528", borderRadius: 10, padding: 12 }}>
        <select value={newCourse} onChange={e => setNewCourse(e.target.value)} style={{ background: "#0D0D0F", border: "1px solid #252528", borderRadius: 6, color: "#E8E8ED", fontSize: 12, padding: "4px 6px", outline: "none", fontFamily: "JetBrains Mono, monospace" }}>
          {Object.values(COURSES).map(c => <option key={c.id} value={c.id}>{c.code}</option>)}
        </select>
...
const NAV = [
  ...
  { id: "gpa", label: "GPA Calc", icon: "calc" },
...
export default function App() {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#0D0D0F", color: "#E8E8ED", fontFamily: "'Inter', -apple-system, sans-serif" }}>
      {/* sidebar nav */}
      <div style={{ width: 220, borderRight: "1px solid #1A1A1E", display: "flex", flexDirection: "column", padding: "20px 12px" }}>
        ...
        <div style={{ marginTop: "auto", padding: "0 8px" }}>
          ...
          <div style={{ fontSize: 11, color: "#4A4A52" }}>Phase 1 · v1.0</div>
          <div style={{ fontSize: 10, color: "#3A3A40", marginTop: 2 }}>Dashboard → Phase 2</div>
        </div>
      </div>
